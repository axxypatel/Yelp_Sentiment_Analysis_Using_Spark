...
class MLPreprocessing(spark:SparkSession,reviewDF:DataFrame, modelTraining: Boolean) {

        //Implicit methods available in Scala for converting common Scala objects into DataFrames
        import spark.implicits._
        def getDFBymodelTraining():DataFrame  =
        {
                if (modelTraining) {
        ...
        def reviewPreprocess():DataFrame = {
                val reviewDFSentiment = getDFBymodelTraining()
